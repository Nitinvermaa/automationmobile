...
public class VerifyMCMDeviceBrandingFail extends BaseTest {
    static Logger LOGGER = Logger.getLogger(VerifyMCMDeviceBrandingFail.class.getName());
    ...
    @HeaderData(
            testCaseUniqueId = "MCM_2",
            featureName = "Device Branding",
            executableFor = {Platforms.ANDROID},
            testDescription = "Verify MCM OOBE flow failed scenario",
            testCaseId = "MCM_2",
    ...
    @Test(priority = 1)
    public void verifyMCMOOBEFlowFailed() throws VerificationFailException, InterruptedException, IOException, SQLException, ClassNotFoundException {
        ...
        //installedPkgs.add(0,"com.amazon.mShop.android.shoppingg");
        ...
        androidMCMAdminScreen.getNotificationCMLabel().isFound(3);

        ;
        while (!androidMCMAdminScreen.getApplicationInstalledLabel().isFound(2)){
            System.out.println("--------- Waiting for Application to install -----------");
            //Thread.sleep(8000L);
            if (androidMCMAdminScreen.getApplicationInstalledLabel().isFound(2)){
        ...
        Verify.verifyTrue(androidMCMAdminScreen.getInstalledLabelApplicationTitle().isFound(5),
        ...
        Verify.verifyTrue(androidMCMAdminScreen.getTMobilePlayLabel().isFound(5),
...
//            Verify.verifyTrue(elements.get(j).toString().equals("com.amazon.mShop.android.shoppin")
//                    , "Verify package Installed:: "+ elements.get(j).toString());
//
//                    }
        Verify.verifyTrue("com.amazon.mp3".equals("com.amazon.mp")
                , "Verify package Installed:: "+ "com.amazon.mp3");
        /*for (JsonNode element : elements) {


            if (element.toString().replace("\"", "").contains(installedPkg.toString())) {

                System.out.println("\n" + installedPkg.toString());
                System.out.println("Package From response Json:: " + element);
            }
        }*/
