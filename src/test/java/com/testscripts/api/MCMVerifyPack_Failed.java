...
public class MCMVerifyPack_Failed extends BaseTest {
    static Logger LOGGER = Logger.getLogger(MCMVerifyPack_Failed.class.getName());
    ...
    @HeaderData(
            testCaseUniqueId = "MCM_API_03",
            featureName = "MCM server registration",
            executableFor = {Platforms.API},
            testDescription = "Verification of PCM Pack response fail scenario ",
            testCaseId = "MCM_API_03",
            requirementID = "Req_ver_0.0.1_ID-3.0.0",
            suite = {Suites.REGRESSION, Suites.P1}
    )
    @Test(priority = 1)
    public void validateMCMRegistrationContentFailure() throws VerificationFailException, SQLException, ClassNotFoundException, IOException {
        ...
        LOGGER.info("Triggering API");
        //Passing complete URL
        response = getApiDriver().postRequest("https://mobilesolutionsuzudev.t-mobile.com/MCM/oem/v1/device/register",
                header, body);
        //The status code is being extracted from response
